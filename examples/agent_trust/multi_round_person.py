...
def match_and_compare_numbers_v2(text):
    ...
    for match in matches:
        num_str = match[1] if match[1] else match[3]  # Adjusted to the new grouping
...
def classmate(
    ...
    if first_round:
        player_1_response = player_1.step(str_mes(first_round_prompt + grantee)).msgs[0]
        ans = match_and_compare_numbers_v2(player_1_response.content)
        if ans:
            given_num = ans
        else:
            given_num = extract_unique_decimal(
                cri_agent.step(str_mes(player_1_response.content)).msgs[0].content
            )

        money_prompt = money_prompt.format(
            give=given_num, N=given_num * k, left=10 - given_num
        )
        player_2_response = player_2.step(str_mes(money_prompt + grantee)).msgs[0]
    else:
        player_1_response = player_1.step(str_mes(first_prompt + grantee)).msgs[0]
    ...
    player_1.update_messages("assistant", player_1_response)
    player_2.update_messages("assistant", player_2_response)
...
def multi_round(
    ...
    for k in range(3, 4):
        ...
        for group_num in tqdm.trange(0, len(cha), 2):
            ...
            existed_res = [item for item in os.listdir(save_path) if ".json" in item]
            if check_file_if_exist(existed_res, save_file_check):
                print(save_file_check + "is exist")
                continue
            for i in tqdm.tqdm(range(round_num)):
                input_record[f"round_{i}_input"] = [first_prompt, second_prompt]
